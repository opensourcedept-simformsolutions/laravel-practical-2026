...
class VisitorStatusNotification extends Notification
{
    ...
    {
        $status = ucwords(str_replace('_', ' ', $this->visitorLog->status));
    ...
    public function toMessage(object $notifiable): string
    {
        ...
        if ($status === 'approved') {
            return "Visitor {$visitorName} has been approved by {$approverName} for flat {$flatName}.";
        } elseif ($status === 'rejected') {
            return "Visitor {$visitorName} has been rejected by {$approverName} for flat {$flatName}.";
        } elseif ($status === 'entered') {
            return "Visitor {$visitorName} has entered the society (approved by {$approverName}) for flat {$flatName}.";
