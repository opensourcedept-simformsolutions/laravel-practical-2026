...
@push('scripts')
    <script>
        ...
        $(document).on('change', '#flat-filter, #type_filter, #status-filter', function() {
            updateFilterBadges();
        });
        ...
        function updateFilterBadges() {
            ...
            let typeVal = $('#type_filter').val();
            if (typeVal) {
                let typeText = $('#type_filter option:selected').text().trim();
                list.append(`
                    <span class="badge bg-light text-dark border d-inline-flex align-items-center gap-1 py-1.5 px-3 rounded-pill">
                        Type: ${typeText}
                        <span class="ms-1 remove-filter-btn text-danger fw-bold" style="cursor: pointer; font-size: 1rem; line-height: 1;" data-filter="type">&times;</span>
                    </span>
                `);
                count++;
            }
        ...
        $(document).on('click', '.remove-filter-btn', function() {
            ...
            if (filterType === 'society') {
                $('#society_filter').val('').trigger('change');
            } else if (filterType === 'soft_delete') {
                $('#status-filter').val('all').trigger('change');
            } else if (filterType === 'wing') {
                $('#wing_filter').val('').trigger('change');
