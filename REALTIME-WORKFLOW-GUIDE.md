...
* **Real-time Push Server:** Laravel Reverb (WebSocket protocol running on port `9000`)
...
We run Laravel Reverb on port `9000` (`REVERB_SERVER_PORT=9000`).
...
1. **Verify WebSocket Connection:**
   * Run `php artisan reverb:start --port=9000`
   * Check status: `curl -i http://localhost:9000`
